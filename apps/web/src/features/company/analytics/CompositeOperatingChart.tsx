...
export default function CompositeOperatingChart({ days, weeks, overlays, compact = false }: { days: OperatingDayPoint[]; weeks: OperatingWeekPoint[]; overlays: CalendarOverlay[]; compact?: boolean }) {
  ...
  const volumeMax = volumeCeiling(Math.max(...points.flatMap((point) => [point.stops, point.packages]), 1));
  const routeMax = Math.max(...points.map((point) => point.routes), 1);
  ...
  return <article className="app-card" style={{ padding: 0, overflow: "hidden" }}>
    <div style={{ padding: 18, display: "flex", justifyContent: "space-between", gap: 18, flexWrap: "wrap" }}><div><p className="value-card__eyebrow">Demand and deployed capacity</p><h2 className="app-card__title" style={{ fontSize: compact ? 21 : 27 }}>Operating relationship</h2><p className="app-card__body" style={{ marginTop: 5 }}>Stops form the operating load, packages show handling volume, and route bubbles show capacity placed against demand.</p></div><div style={{ display: "flex", gap: 12, alignItems: "end", flexWrap: "wrap" }}><div style={{ display: "flex", gap: 4 }}>{(["week","month"] as Grain[]).map((item) => <button key={item} className={`app-button ${grain === item ? "app-button--primary" : "app-button--secondary"}`} style={{ minHeight: 32, padding: "5px 9px" }} onClick={() => setGrain(item)}>{item[0].toUpperCase()+item.slice(1)}</button>)}</div>{(["stops","packages","routes"] as const).map((layer) => <label key={layer} style={{ fontSize: 12, fontWeight: 800, textTransform: "capitalize" }}><input type="checkbox" checked={layers[layer]} onChange={() => setLayers((current) => ({ ...current, [layer]: !current[layer] }))} /> {layer}</label>)}</div></div>
    <div style={{ position: "relative", padding: "0 10px 14px" }}>{activePoint ? <div style={{ position: "absolute", zIndex: 3, top: 10, right: 22, padding: "10px 12px", border: "1px solid #cbd5e1", borderRadius: 10, background: "rgba(255,255,255,.96)", boxShadow: "0 8px 24px rgba(15,23,42,.12)", fontSize: 12 }}><strong>{activePoint.label}</strong><div>{fmt(activePoint.stops)} stops · {fmt(activePoint.packages)} packages</div><div>{fmt(activePoint.routes, 1)} average routes/day · {activePoint.operatingDays} operating days</div><div>{fmt(activePoint.packages / Math.max(activePoint.stops, 1), 2)} packages/stop · {fmt(aggregateStopsPerRoute(activePoint.stops, activePoint.routeDays), 1)} stops/route</div>{activePoint.signal ? <div style={{ marginTop: 3, color: "#b45309", fontWeight: 800 }}>{activePoint.signal === "SUPPLEMENTAL_OPERATION" ? "Supplemental operation" : "Possible capacity intervention"}</div> : null}</div> : null}<svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", display: "block" }} onMouseLeave={() => setActive(null)} role="img" aria-label="Stops and packages on a shared volume scale with average routes over time">
