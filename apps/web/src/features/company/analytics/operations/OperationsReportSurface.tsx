  ...
  return <main className="workspace-shell"><section className="workspace-main" style={{ paddingTop: 0, paddingBottom: 36 }}>
    <article style={{ maxWidth: 1180, margin: "0 auto", background: "#fff", border: "1px solid #dbe3ed", boxShadow: "0 18px 50px rgba(15,23,42,.06)", padding: "clamp(22px, 4vw, 48px)" }}>
      ...
      {!payloadLoading && report.operatingDays > 0 ? <>
        ...
        <section style={{ paddingTop: 28 }}><p className="value-card__eyebrow">Weekly operating ledger</p><h2 style={{ margin: "5px 0 14px", fontSize: 28 }}>Chronological operating record</h2><div style={{ overflowX: "auto", maxHeight: 520 }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 820 }}><thead style={{ position: "sticky", top: 0, background: "#fff" }}><tr>{["Week ending", "Days", "Routes", "Stops", "Packages", "Stops / route", "Packages / route", "WoW stops"].map((label) => <th key={label} style={{ textAlign: label === "Week ending" ? "left" : "right", padding: "10px 8px", borderBottom: "2px solid #0f172a", fontSize: 11, textTransform: "uppercase", letterSpacing: ".05em" }}>{label}</th>)}</tr></thead><tbody>{[...report.weeks].reverse().map((week) => <tr key={week.weekStart}><td style={{ padding: "12px 8px", borderBottom: "1px solid #e2e8f0", fontWeight: 700 }}>{shortDate(week.weekEnd)}</td>{[number(week.operatingDays), number(week.routes), number(week.stops), number(week.packages), number(week.stopsPerRoute, 1), number(week.packagesPerRoute, 1), percent(week.stopsChange)].map((value, index) => <td key={index} style={{ padding: "12px 8px", borderBottom: "1px solid #e2e8f0", textAlign: "right", color: index === 6 && week.stopsChange != null ? week.stopsChange >= 0 ? "#166534" : "#b91c1c" : "#334155" }}>{value}</td>)}</tr>)}</tbody></table></div></section>
