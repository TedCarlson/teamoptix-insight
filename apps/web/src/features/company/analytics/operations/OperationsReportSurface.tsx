  ...
  return <main className="workspace-shell"><section className="workspace-main" style={{ paddingTop: 0, paddingBottom: 36 }}>
    <article style={{ width: "100%", background: "#fff", border: "1px solid #dbe3ed", boxShadow: "0 18px 50px rgba(15,23,42,.06)", padding: "clamp(18px, 2.5vw, 36px)" }}>
      <header style={{ borderBottom: "3px solid #0f172a", paddingBottom: 22, display: "flex", justifyContent: "space-between", gap: 24, alignItems: "end", flexWrap: "wrap" }}>
        <div><p className="value-card__eyebrow">Analytics · Operations</p><h1 style={{ margin: "7px 0 0", fontSize: "clamp(34px, 5vw, 54px)", lineHeight: 1, letterSpacing: "-.045em" }}>Operations Report</h1><p style={{ margin: "12px 0 0", color: "#475569", maxWidth: 680, lineHeight: 1.65 }}>A calendar-range reading of demand, route supply, and the workload carried by each operating plan.</p></div>
        <div style={{ textAlign: "right", color: "#475569", fontSize: 12, lineHeight: 1.7 }}><strong style={{ display: "block", color: "#0f172a", fontSize: 14 }}>Calendar year {loadedYear ?? "—"}</strong><span>FINAL DSW · {report.operatingDays} operating days</span><br/><span>Current through {report.throughDate ? shortDate(report.throughDate) : "—"}</span></div>
      </header>
      ...
      {!payloadLoading && !error && payload && !report.operatingDays ? <div style={{ padding: "50px 0" }}><strong>No FINAL operating history is available for this calendar range.</strong></div> : null}
      ...
      {!payloadLoading && report.operatingDays > 0 ? <>
        ...
        <footer style={{ borderTop: "1px solid #cbd5e1", marginTop: 30, paddingTop: 16, color: "#64748b", fontSize: 11 }}>Source: persisted FINAL DSW history for the selected calendar range through {report.throughDate}. Rolling comparisons use operating records within each calendar window; no inferred route detail or independent date scope is introduced.</footer>
      </> : null}
    </article>
  </section></main>;
}
