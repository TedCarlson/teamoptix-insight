...
export default async function Page() {
  ...
  return <TeamOptixDomainOverview eyebrow="TeamOptix · Automation" title="Automation control" description="Collection demand, runner execution, artifact movement, and ingestion outcomes across governed Insight customers."
    ...
    panels={[
      { eyebrow: "Exceptions", title: "Automation attention", actionLabel: "Telemetry", actionHref: "/teamoptix/automation/telemetry", rows: data.failedRequests.length ? data.failedRequests.slice(0, 6).map((row) => ({ title: `${String(row.request_type).replaceAll("_", " ")} failed`, detail: `${String(row.company_slug)} · ${String(row.error_message ?? "Inspect collection evidence")}`, status: "Failed", href: "/teamoptix/automation/collections" })) : data.failedRuns.length ? data.failedRuns.slice(0, 6).map((row) => ({ title: `${String(row.automation_type).replaceAll("_", " ")} failed`, detail: `${String(row.company_slug)} · ${String(row.error_message ?? "Inspect recorded run")}`, status: "Failed", href: "/teamoptix/automation/telemetry" })) : [{ title: "No recorded failures", detail: "No failed requests or runner executions in the trailing seven days", status: "Clear", href: "/teamoptix/automation/telemetry" }] },
