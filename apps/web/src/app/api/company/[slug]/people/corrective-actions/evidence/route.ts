...
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  ...
  const routes = rows.map((row: any, index: number) => {
    ...
    return {
      ...
      route_baseline_id: row.route_baseline_id ?? null,
      ...
      early_late_pickups_raw: row.early_late_pickups_raw ?? null,
      early_pickups: Number(row.early_pickups ?? 0),
      late_pickups: Number(row.late_pickups ?? 0),
