-- Driver resource assignment bridge.
-- Enforces one active asset per roster member and asset type while preserving
-- assignment history. Used by both Assets and the roster drawer.
...
begin
  ...
  -- Release any other asset occupying this driver's slot for the same type.
...
create or replace function public.assign_company_asset_to_roster_slot(
  p_company_slug text,
  p_asset_id uuid,
  p_roster_member_id uuid
) returns jsonb
...
as $$
  select core.assign_company_asset_to_roster_slot(
    p_company_slug,
    p_asset_id,
    p_roster_member_id
  );
...
revoke all on function
  public.assign_company_asset_to_roster_slot(text, uuid, uuid)
from public;
...
comment on function
  core.assign_company_asset_to_roster_slot(text, uuid, uuid)
is
  'Assigns an asset to a driver-facing roster slot, releasing any prior active asset of the same type for that roster member.';
