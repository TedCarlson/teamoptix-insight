...
declare v_company_id uuid; v_roster_id uuid; v_inspection_id uuid; v_item jsonb; v_item_id uuid; v_has_defect boolean := false; v_media text[];
begin
  ...
  insert into fleet.inspection(company_id,vehicle_id,driver_roster_member_id,inspection_type,record_class,status,odometer_miles,submitted_at,safe_to_operate_driver,driver_notes,route_name)
  values(v_company_id,p_vehicle_id,v_roster_id,p_inspection_type,case when v_has_defect then 'DVIR_DEFECT_REPORT' else 'COMPANY_INSPECTION' end,'SUBMITTED',p_odometer_miles,now(),p_safe_to_operate,nullif(btrim(p_driver_notes),''),nullif(btrim(p_route_name),'')) returning id into v_inspection_id;
  for v_item in select * from jsonb_array_elements(coalesce(p_items,'[]'::jsonb)) loop
    select coalesce(array_agg(value), '{}'::text[]) into v_media from jsonb_array_elements_text(coalesce(v_item->'media_paths','[]'::jsonb));
    insert into fleet.inspection_item(inspection_id,company_id,section_key,item_key,item_label,result,notes,media_paths)
    values(v_inspection_id,v_company_id,v_item->>'section_key',v_item->>'item_key',v_item->>'item_label',v_item->>'result',nullif(v_item->>'notes',''),v_media) returning id into v_item_id;
    if v_item->>'result'='DEFECT' then
      insert into fleet.defect(company_id,vehicle_id,inspection_id,inspection_item_id,reported_by_roster_member_id,category,summary,description,severity,safe_to_operate_driver,media_paths)
      values(v_company_id,p_vehicle_id,v_inspection_id,v_item_id,v_roster_id,v_item->>'section_key',v_item->>'item_label',nullif(v_item->>'notes',''),case when p_safe_to_operate then 'REPAIR_SOON' else 'UNSAFE_OUT_OF_SERVICE' end,p_safe_to_operate,v_media);
    end if;
  end loop;
