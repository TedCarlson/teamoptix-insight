  ...
  loop
    ...
    v_route_key := case
      when v_manifest_type = 'delivery'
        then substring(v_artifact.original_filename from '^\d{8}_(\d{3,4})\s+')
    ...
    if v_manifest_type is null or nullif(v_route_key, '') is null then
      update core.operations_collection_artifact
      set
        artifact_status = 'FAILED',
        error_message = 'Unable to infer manifest type or route key for promotion.',
        ingest_metadata_json = coalesce(ingest_metadata_json, '{}'::jsonb) || jsonb_build_object(
          ...
          'failed_at', now(),
          'reason', 'UNABLE_TO_INFER_MANIFEST_ROUTE'
        ),
        updated_at = now()
    ...
    v_route_label := case
      when v_manifest_type = 'delivery'
        then regexp_replace(v_artifact.original_filename, '^\d{8}_\d{3,4}\s+', '')
      when v_manifest_type = 'pickup'
        then 'WA ' || v_route_key
      else v_route_key
    end;
    ...
    where p.company_id = v_artifact.company_id
      and p.service_date = v_artifact.service_date
