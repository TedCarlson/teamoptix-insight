alter table core.company_operations_ticket_assignment
  drop constraint if exists company_operations_ticket_assignment_cadence_chk;

alter table core.company_operations_ticket_assignment
  add constraint company_operations_ticket_assignment_cadence_chk check (
    cadence_minutes is null
    or cadence_minutes between 5 and 1440
  );
...
begin
  ...
  if p_cadence_minutes is not null and (p_cadence_minutes < 5 or p_cadence_minutes > 1440) then
    raise exception 'Cadence must be blank or between 5 minutes and 24 hours.';
  end if;
