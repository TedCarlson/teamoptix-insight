...
create or replace function core.itf_write_onboarding_roster_identity(
  p_roster_id uuid,
  p_full_name text,
  p_email text,
  p_phone text,
...
begin
  ...
  loop
    ...
    values (
      ...
      case when v_identifier.identifier_type = 'tech_id'
        then upper(v_identifier.identifier_value)
        else v_identifier.identifier_value
      end
    );
  end loop;
end;
...
revoke all on function core.itf_write_onboarding_roster_identity(
...
begin
  ...
  insert into core.company_roster_event (
    ...
    'Roster identity updated without changing the FUSE onboarding lifecycle or workforce placement.',
    jsonb_build_object(
  ...
  return jsonb_build_object('ok', true, 'roster_id', v_candidate.roster_id, 'candidate_id', v_candidate.candidate_id);
end;
$$;
...
begin
  ...
  if p_placement not in ('training', 'field') then raise exception 'Choose Training or Active Field.'; end if;
  ...
  v_seat_type := case when p_placement = 'training' then 'TRAINING' else 'FIELD' end;
  v_employment_status := case when p_placement = 'training' then 'Trainee' else 'Active' end;
  ...
  set worker_type = 'TECH', job_title = 'Technician',
      employment_status = v_employment_status, seat_type = v_seat_type,
  ...
  insert into core.company_roster_event (
    ...
    'Started onboarding candidate placed into the ITF workforce by an authorized operator.',
    jsonb_build_object(
      'workspace_company_id', v_candidate.workspace_company_id,
      'candidate_id', v_candidate.candidate_id,
