...
async function fetchRowsForRegion(sb: any, batch_id: string, region_name: string) {
  ...
  for (const p of probes) {
    const { data, error } = await sb
      ...
      .eq(p.col, region_name);
    if (!error) return { data: data ?? [], usedCol: p.col };
  }

  return { data: [], usedCol: null as string | null };
}

/**
 * GET /api/region/techs?region=Keystone
 * Optional:
...
export async function GET(req: Request) {
  try {
    ...
    const region_name = s(url.searchParams.get("region"));
    if (!region_name) {
    ...
    // 2) Pin for that batch (optional)
    const { data: pin, error: pinErr } = await sb
      .from(PINS_TABLE)
      .select("batch_id,fiscal_month_anchor,scope,source_system,rubric_version_id,settings_pinned_at,created_at")
      .eq("batch_id", batch_id)
      .limit(1)
      .maybeSingle();

    if (pinErr) return NextResponse.json({ ok: false, error: pinErr.message }, { status: 500 });
    ...
    return NextResponse.json({
      ...
      debug: {
        region_column_used: rowsResult.usedCol,
      },
    });
  } catch (e: any) {
