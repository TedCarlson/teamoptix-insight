...
export async function GET(req: NextRequest) {
  ...
  const sendOnly = sendEmail && mode === "send";

  // ---- v1 hardcoded scope (demo) ----
  const region = "Keystone";
  const fiscalMonth = "2025-12-21";
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .wrap { padding: 16px; }
          .header-grid { display: grid; grid-template-columns: 1.2fr 1fr 1fr 1fr; gap: 10px; margin-bottom: 10px; align-items: stretch; }
          .hdr { border: 1px solid #ddd; border-radius: 6px; padding: 12px 12px 14px; height: 68px; box-sizing: border-box; }
          .hdr-title { font-size: 10px; text-transform: uppercase; letter-spacing: 0.6px; color: #666; margin-bottom: 4px; }
          .hdr-value { font-size: 14px; font-weight: 700; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
          .hdr-sub { font-size: 10px; color: #444; }
          .meta-row { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 10px; margin: 4px 0 10px 0; font-size: 10px; color: #222; }
          .meta-pill { border: 1px solid #eee; border-radius: 999px; padding: 6px 10px; display: inline-block; width: fit-content; }
          .meta-pill b { color: #000; }
          table { width: 100%; border-collapse: collapse; table-layout: fixed; }
          thead th { border-bottom: 2px solid #222; border-top: 1px solid #ddd; padding: 8px 8px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
          tbody td { border-bottom: 1px solid #eee; padding: 5px 6px; vertical-align: middle; word-wrap: break-word; }
          ...
          .footer { margin-top: 10px; font-size: 9px; color: #555; line-height: 1.4; }
        </style>
      </head>
      <body>
        <div class="wrap">
          ...
          <div class="meta-row">
            <div class="meta-pill"><b>tNPS:</b> ${escapeHtml(fmt1((regionPdfKpis as any).tnps_rate))}</div>
            <div class="meta-pill"><b>FTR:</b> ${escapeHtml(fmt1((regionPdfKpis as any).ftr_pct))}</div>
            <div class="meta-pill"><b>Tool Usage:</b> ${escapeHtml(fmt1((regionPdfKpis as any).tool_usage_pct))}</div>
            <div class="meta-pill"><b>Headcount:</b> ${escapeHtml(String((regionPdfKpis as any).headcount))}</div>
          </div>
  ...
  try {
    ...
    // ✅ If this request is "send-only", return JSON so the browser doesn't open PDF
    if (sendOnly) {
      return NextResponse.json({ ok: true, emailed: true, region, fiscalMonth }, { status: 200 });
    }

    // Otherwise: return PDF preview in browser
